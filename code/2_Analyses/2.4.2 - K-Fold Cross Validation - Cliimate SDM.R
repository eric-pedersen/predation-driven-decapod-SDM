### K-Fold Cross validation for Climate Only SDMs ----
...
# Abiotic data
Abiotic_Data<-fread('data/combined data/Abiotic_Data_Allregions.csv')

# RV data
RV_Data<-fread('data/combined data/RV_Data_Allregions.csv')
...
Data_Arctic$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Arctic,type="response")
...
Data_Gulf$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Gulf,type="response")
Data_Gulf$pred_crab<-predict(climate_crabmodel,newdata=Data_Gulf,type="response")
...
Data_Maritimes$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Maritimes,type="response")
Data_Maritimes$pred_crab<-predict(climate_crabmodel,newdata=Data_Maritimes,type="response")
...
Data_Quebec$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Quebec,type="response")
Data_Quebec$pred_crab<-predict(climate_crabmodel,newdata=Data_Quebec,type="response")
...
Data_Newfoundland_fall$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Newfoundland_fall,type="response")
Data_Newfoundland_fall$pred_crab<-predict(climate_crabmodel,newdata=Data_Newfoundland_fall,type="response")
...
Data_Newfoundland_spring$pred_shrimp<-predict(climate_shrimpmodel,newdata=Data_Newfoundland_spring,type="response")
Data_Newfoundland_spring$pred_crab<-predict(climate_crabmodel,newdata=Data_Newfoundland_spring,type="response")
...
gam_model_group1 <- gam(pandalus_borealis~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Shrimp[rand.vec!=1],method="REML")
gam_model_group2 <- gam(pandalus_borealis~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Shrimp[rand.vec!=2],method="REML")
gam_model_group3 <- gam(pandalus_borealis~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Shrimp[rand.vec!=3],method="REML")
gam_model_group4 <- gam(pandalus_borealis~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Shrimp[rand.vec!=4],method="REML")
gam_model_group5 <- gam(pandalus_borealis~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Shrimp[rand.vec!=5],method="REML")
...
# Fit the model in all folds but one.
gam_model_group1 <- gam(chionoecetes_opilio_small~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Crab[rand.vec!=1],method="REML")
gam_model_group2 <- gam(chionoecetes_opilio_small~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Crab[rand.vec!=2],method="REML")
gam_model_group3 <- gam(chionoecetes_opilio_small~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Crab[rand.vec!=3],method="REML")
gam_model_group4 <- gam(chionoecetes_opilio_small~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Crab[rand.vec!=4],method="REML")
gam_model_group5 <- gam(chionoecetes_opilio_small~s(temperature_at_bottom,k = 6)+s(sqrt(depth),k=6)+s(region,year_surv,bs="re",by=dummy),family=tw(),data=Data_Crab[rand.vec!=5],method="REML")
...
rm(list=setdiff(ls(), c("Data","shrimp_model_deviance","crab_model_deviance","get_deviance")))
